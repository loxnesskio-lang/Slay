...
async function generateImageGemini(prompt, style, referenceImages = [], options = {}) {
    ...
    // Google AI Studio uses x-goog-api-key header for auth
    const response = await robustFetch(url, { method: 'POST', headers: { 'x-goog-api-key': settings.apiKey, 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
    if (!response.ok) { const text = await response.text(); throw new Error(`API Error (${response.status}): ${text}`); }
    const result = await response.json();
