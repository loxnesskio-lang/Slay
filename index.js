...
async function fetchModels() {
    ...
    const isGeminiApi = settings.apiType === 'gemini';
    if (isGeminiApi) {
        // Google AI Studio uses ?key= param, not Bearer auth.
        // Return known image generation models directly — Google doesn't expose
        // a /v1/models endpoint that lists image models via API key auth.
...
async function generateImageGemini(prompt, style, referenceImages = [], options = {}) {
    const settings = getSettings();
    const model = settings.model;
    const url = `${settings.endpoint.replace(/\/$/, '')}/v1beta/models/${model}:generateContent?key=${encodeURIComponent(settings.apiKey)}`;
    ...
    // Key is passed as ?key= query param in the URL, no Authorization header needed for Google AI Studio
    const response = await robustFetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
    if (!response.ok) { const text = await response.text(); throw new Error(`API Error (${response.status}): ${text}`); }
    const result = await response.json();
